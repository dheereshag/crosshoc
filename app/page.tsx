...
import { getVisibleGames, titleBySection, legacyToCurrentSortMap } from "@/lib/catalog-utils";
...
export default async function HomePage({ searchParams }: PageProps) {
  ...
  const normalizedSort = rawSort ? (legacyToCurrentSortMap[rawSort] ?? rawSort) : undefined;
  const sort = (normalizedSort as SortKey | undefined) ?? "relevance";
  const visibleGames = getVisibleGames(section, genre, sort);
  const title = genre ? `${genre} Games` : (titleBySection[section] ?? "All Games");
    ...
    <div className="px-6 pb-10 pt-2 md:px-8">
      <section>
        <div className="mb-4 flex flex-col gap-3">
          ...
          <FilterBar searchParams={resolvedSearchParams} genre={genre} sort={sort} />
        </div>
